...
function ensureThumbs(){
  ...
  const step = () => {
    // a video recording owns the main thread: every frame this steals is a dropped frame
    if (window.__gfxRecording){ setTimeout(step, 300); return; }
...
function buildLanding(){
  ...
  // …and not one the video reel already shows two sections down
  const inReel = new Set([...document.querySelectorAll('#lp-vreel [data-tpl]')].map(f => f.dataset.tpl));
  const pool = usable.filter(t => !inReel.has(t.id));
  const eligible = varietyOrder(pool.filter(complete).length >= 3
    ? pool.filter(complete) : pool);
  const pick = [];
  /* The front card is a VIDEO when a square clip exists (LANDING v2): it is
     seeded first so the other two are picked not to clash with it, and it is
     pinned to hc3 below, the one slot nothing overlaps. */
  const clip = landingHeroClip();
  const clipTpl = clip && TEMPLATES.find(t => t.id === clip.tpl && !tplLocked(t));
  if (clip && !clipTpl) console.warn('GraphicsStudio: hero clip template ' + clip.tpl + ' missing or locked, the fan falls back to stills.');
  if (clipTpl) pick.push(clipTpl);
  ...
  if (fan.length === 3){
    ...
    // …unless there is a clip: motion outranks colour for the front slot
    if (clipTpl && fan.includes(clipTpl)) fan = fan.filter(t => t !== clipTpl).concat(clipTpl);
  }
  ...
  document.querySelectorAll('#hero-stack .hero-card').forEach((card, i) => {
    if (clipTpl && heroIds[i] === clipTpl.id && i === 2){
      /* buildLanding re-runs every time a batch of photos decodes; the video
         element is memoised and left in place, or it would restart each time. */
      const v = landingVideoEl(clip, clipTpl.name);          // poster = frame 0 = the finished ad
      if (v.parentNode !== card){ card.innerHTML = ''; card.appendChild(v); }
    } else {
      card.innerHTML = '';
      const img = new Image();
    ...
    card.onclick = e => { if (e.target.closest('.vad-sound')) return; showEasy(heroIds[i]); };
  });
  ...
  // Gallery: renderLandingGallery (LANDING v2, end of file) owns the shop
  // window's order, the category filter and the lock labels now.
  window.__PRIORITY_TPL_IDS = heroIds.concat(window.__PRIORITY_TPL_IDS || []);
...
function loadTemplate(id){
  ...
  if (_t && tplLocked(_t)){ openPlans(proPitch(_t.name)); return; }
  ...
  if (filter !== 'mine'){
    TEMPLATES.filter(t => filter === 'all' ? t.cat === currentCat : t.cat === filter).forEach(t => {
      ...
      const c = pickerCard(getThumb(t.id, 320), t.name, locked ? 'premium 🔒' : t.tag,
        () => { if (tplLocked(t)){ openPlans(proPitch(t.name)); return; } closePicker(); loadTemplate(t.id); });
...
const TUT_STEPS = [
  ...
  { sel:'#badge-chips',     pos:'right',  title:'Selling points', body:'One tap adds selling points like QUICK, SAFE or SAME DAY. Three or four is plenty; more reads as noise.' },
  ...
  { sel:'#panel-right',    pos:'left',  title:'Fine-tune everything', body:'Properties for colour, size and spacing, plus a Layers tab to reorder, hide or lock elements.' },
  { sel:'#export-btn',     pos:'bottom', title:'Export & post', body:'Download a crisp PNG or an 8-second video in your chosen format: square for Marketplace & Instagram, story for Reels, or a print-ready 8.5×11 flyer. That\'s the whole workflow.' },
];
let tutIdx = 0;
...
function refreshPhotoThumb(tplId){
  ...
  setTimeout(function repaint(){      // batch: one repaint even if all 8 land together
    if (window.__gfxRecording){ setTimeout(repaint, 300); return; }   // never during a video recording
...
function resetEzProject(){
  ...
  ez.bg = null; ez.bgData = null; ez.bgImgObj = null; ez.bgRecId = null; ez.bgPicked = true;
...
function refreshPlanFeats(){
  try {
    ...
    if (typeof PLANS === 'object' && PLANS){
      ...
      // the quota line is written FROM the quota, not beside it (rule 37)
      Object.values(PLANS).forEach(p => {
        p.feats[0] = (p.weekly ? p.weekly + ' exports per week' : p.monthly + ' exports per month') + ', image or video';
      });
    }
  } catch (e){}
}
...
const PLANS = {
  free: { label:'Free', price:0, priceLabel:'$0', per:'forever', maxPx:1080, watermark:true, weekly:3, monthly:null,
          feats:['3 exports per week, image or video',FEAT_FREE_TPL,'1080px downloads, every format','BUYBACK.AD watermark'] },
  pro:  { label:'Pro', price:15, priceLabel:'$15', per:'/month', maxPx:2160, watermark:false, weekly:null, monthly:100,
          feats:['100 exports per month, image or video',FEAT_PRO_TPL,'Up to 2160px, no watermark, every format','Export history & re-downloads'], hot:true },
};
...
function applyWatermark(dataUrl, w, h){
  ...
  return new Promise(res => {
    ...
    im.onload = () => {
      ...
      // each corner mark rotated 45° along its corner→center diagonal; shared
      // with the video exporter (drawWatermarkMarks, MOTION section)
      drawWatermarkMarks(x, w, h);
...
function refreshEzLayers(){
  ...
  const sw = isPhoto ? `<img src="${ez.bgData}" alt="">`
    : isTplPhoto ? `<img src="${tplPhotoUrl(bgSpec.src)}" alt="">`
...
let ez = { tpl: null, vals:{}, chips:null, bg:null, bgPicked:true, custom:'#2563eb', hidden:{}, bgRecId:null, bgData:null, bgImgObj:null, fx:{ blur:0, overlay:'none', oc:'#000000', os:45 }, styles:{}, customPoints: jget('pgfx_custom_points', []) };
...
function refreshTplLocks(){
  document.querySelectorAll('.ez-tpl').forEach(b => {
    ...
    const label = b.querySelector('span:not(.pro-tag)');
    let tag = b.querySelector('.pro-tag');
    if (locked && !tag && label){ tag = document.createElement('span'); tag.className = 'pro-tag'; tag.textContent = 'Pro'; label.appendChild(tag); }
...
function tplPhotoUrl(src){
  const el = TPL_BG_ELS[src];
  return (el && el.src) || (window.TPL_BG_DATA || {})[src] || src;
}
function cssBg(spec){
  if (spec.type === 'image'){
    /* Resolve the photo the way the canvas does: the decoded element, then the
       embedded copy, then the path. The classic set's paths point at
       assets/tplbg/, which is not shipped (they live in tplbg-data.js), so the
       bare path 404'd and the layer swatch drew nothing. */
    if (spec.src) return `url("${tplPhotoUrl(spec.src)}") center/cover`;
...
function buildEzStrip(){
  ...
  catTemplates().forEach(t => {
    ...
    /* The PRO mark is decided by refreshTplLocks() from tplLocked(), the
       predicate the gate actually uses. It used to be `t.tier === 'premium'`,
       which labelled every Phones design PRO although every one is free, and
       it sat on the top-left of the art, over the headline. */
...
function selectEzTpl(id){
  ...
  /* The template's own photograph IS the default. This used to reset to a
     greyscale, blurred placeholder that exported on a flat fallback unless the
     visitor clicked "ORIG" — which the swatch row already showed as selected.
     Someone who tapped a full-colour gold ad on the landing page arrived at a
     grey one. The house direction is that every template keeps its photograph. */
  ez.bgPicked = true;
...
/* The Easy Mode scene, built but not rasterised. Split out of renderEzCanvas
   so the video exporter records exactly the ad the preview shows — one
   builder, two outputs (a PNG, or frames), never two copies of this logic. */
function buildEzScene(mode){
...
function boot(){
  ...
  const clipsReady = Promise.race([CLIP_MANIFEST, new Promise(r => setTimeout(() => r([]), 2500))]).then(applyClipManifest);
  Promise.all([ensureTemplateFonts(), clipsReady]).then(() => {
    ...
    /* The preloader orders wave 1 by __PRIORITY_TPL_IDS, but buildLanding()
       (which used to set it) runs AFTER the preloader, so wave 1 never saw
       the landing picks. The showcase order is cheap to compute, so compute it
       first; buildLanding() prepends the hero fan when it runs. */
    try { window.__PRIORITY_TPL_IDS = landingShowcase('all').slice(0, LP_PAGE).map(t => t.id); } catch (e){}
    preloadTplBgs();          // photos start downloading immediately…
    preloadCutouts();         // …as do the street family's product cutouts…
    buildLanding();
    try { refreshPlanFeats(); refreshCountCopy(); } catch (e){}   // copy states counts; make them true
    try { buildLandingExtras(); } catch (e){ console.error('GraphicsStudio: landing facts failed to render', e); }           // …while the CPU paints fallbacks, upgraded on arrival
  });
}
...
async function ezDownloadVideo(){
  ...
  const gate = await gateExport(ezExportPx());
  if (!gate) return;
  const px = Math.min(gate.px, MOTION.maxShort);
  const sc = buildEzScene((!ez.bg && !ez.bgPicked) ? 'export' : undefined);
  await runVideoExport({ sc, docW:TPL_W, docH:TPL_H, w:px, h:px, watermark:gate.watermark, actx, name:ezTpl().name, btn:$('ez-video') });
}
...
/* ═══════════════ LANDING v2 ═══════════════
   The page that sells the product, held to the rules the product is held to.
   Everything that states a fact about the library — counts, categories,
   formats, prices, quotas — is written here from TEMPLATES / CATS / FORMATS /
   PLANS at boot (rule 37), so the page cannot drift from the product again. */
const catLabel = id => { const c = CATS.find(x => x.id === id); return c ? c.label.replace(/^\S+\s+/, '') : id; };
/* One sentence for every "this is Pro" moment. Four call sites used to say
   "unlock all 8 designs with Starter or Pro": there is no Starter plan, and
   the number was wrong in every category. */
function proPitch(name){
  const n = (() => { try { return tplCounts().totalRounded; } catch (e){ return 'all'; } })();
  return (name ? '“' + name + '” is a Pro template. ' : '') + 'Pro unlocks all ' + n + ' templates, 2160px exports and no watermark.';
}

// ── gallery ──────────────────────────────────────────────────────────────
let LP_CAT = 'all', LP_SHOWN = 12;
const LP_PAGE = 12;
/* 'all' is the curated shop window (street family, then featured designer
   work, then everything free), interleaved so no two neighbours share a
   layout. A category shows the SAME order Easy Mode shows (orderedCatList),
   so "the first three are free" stays literally true on this page too. */
function landingShowcase(cat){
  let ordered;
  if (cat && cat !== 'all') ordered = orderedCatList(cat);
  else {
    ...
    const seen = new Set();
    ordered = varietyOrder(best.filter(t => !seen.has(t.id) && seen.add(t.id)));
  }
  // unlocked first, so the first cards anyone taps open the editor, not the upsell
  return ordered.filter(t => !tplLocked(t)).concat(ordered.filter(t => tplLocked(t)));
}
function renderLandingGallery(){
  const grid = $('lp-tpl-grid');
  if (!grid) return;
  const list = landingShowcase(LP_CAT);
  const n = Math.min(list.length, LP_SHOWN);
  grid.innerHTML = '';
  list.slice(0, n).forEach(t => {
    ...
    item.innerHTML =
      `<div class="tpl-card" role="button" tabindex="0" aria-label="${escHtml(t.name)}${locked ? ', Pro template' : ', open this template'}">` +
        `<img src="${getThumb(t.id, 320)}" alt=""><div class="tpl-use">${locked ? 'See Pro' : 'Use template'} →</div></div>` +
      `<div class="tpl-cap"><span class="tpl-name">${escHtml(t.name)}</span>` +
        (locked ? '<span class="tpl-tag pro">Pro</span>' : `<span class="tpl-tag">${escHtml(catLabel(t.cat))}</span>`) + '</div>';
    const card = item.querySelector('.tpl-card');
  ...
  if (more){
    ...
    more.textContent = 'Show ' + Math.min(LP_PAGE, list.length - n) + ' more';
    more.onclick = () => { LP_SHOWN += LP_PAGE; renderLandingGallery(); };
  }
}
function setLandingCat(cat, scroll){
  LP_CAT = cat; LP_SHOWN = LP_PAGE;
  document.querySelectorAll('#lp-filter .lp-chip').forEach(b => b.setAttribute('aria-pressed', String(b.dataset.cat === cat)));
  renderLandingGallery();
  if (scroll) $('lp-templates').scrollIntoView({ behavior:'smooth', block:'start' });
}
function buildLandingFilter(){
  const f = $('lp-filter');
  if (!f) return;
  const counts = {};
  TEMPLATES.forEach(t => { counts[t.cat] = (counts[t.cat] || 0) + 1; });
  const chips = [{ id:'all', label:'Best of' }]
    .concat(CATS.filter(c => counts[c.id]).map(c => ({ id:c.id, label:catLabel(c.id), n:counts[c.id] })));
  f.innerHTML = chips.map(c => `<button class="lp-chip" type="button" data-cat="${c.id}" aria-pressed="${c.id === LP_CAT}">` +
    escHtml(c.label) + (c.n ? ` <span class="num">${c.n}</span>` : '') + '</button>').join('');
  f.onclick = e => { const b = e.target.closest('.lp-chip'); if (b) setLandingCat(b.dataset.cat); };
}

// ── facts, from the source of truth ─────────────────────────────────────
function fillLandingFacts(){
  const c = tplCounts();
  const cats = CATS.filter(x => TEMPLATES.some(t => t.cat === x.id));
  const set = (k, v) => document.querySelectorAll('[data-count="' + k + '"]').forEach(el => { el.textContent = v; });
  set('templates', c.totalRounded);
  set('categories', String(cats.length));
  set('formats', String(Object.keys(FORMATS).length));
  // hero category list: read as text, and each one filters the gallery
  const hc = $('lp-hero-cats');
  if (hc){
    hc.innerHTML = cats.map(x => `<li><button type="button" data-cat="${x.id}">${escHtml(catLabel(x.id))}</button></li>`).join('');
    hc.onclick = e => { const b = e.target.closest('button[data-cat]'); if (b) setLandingCat(b.dataset.cat, true); };
  }
  const labels = cats.map(x => catLabel(x.id));
  document.querySelectorAll('[data-cat-list]').forEach(el => {
    el.textContent = labels.slice(0, -1).join(', ') + ' and ' + labels[labels.length - 1];
  });
  // formats, straight from FORMATS
  const row = $('lp-formats');
  if (row) row.innerHTML = Object.values(FORMATS).map(f =>
    `<div class="fmt"><div class="fmt-frame"><i style="aspect-ratio:${f.w}/${f.h}"></i></div>` +
    `<b>${escHtml(f.label)}</b><span>${escHtml(f.hint)}</span><span class="num">${f.w} × ${f.h}</span></div>`).join('');
  // pricing, straight from PLANS (the plans page reads the same object)
  const g = $('lp-price-grid');
  if (g){
    g.innerHTML = '';
    Object.entries(PLANS).forEach(([id, p]) => {
      const card = document.createElement('div');
      card.className = 'lp-price-card' + (p.hot ? ' hot' : '');
      card.innerHTML = `<h3>${escHtml(p.label)}</h3>` +
        `<div class="lp-price">${escHtml(p.priceLabel)}<span class="lp-price-per"> ${escHtml(p.per)}</span></div>` +
        `<ul>${p.feats.map(x => '<li>' + escHtml(x) + '</li>').join('')}</ul>` +
        `<button class="btn ${id === 'free' ? 'btn-outline' : 'btn-primary'}" type="button">${id === 'free' ? 'Start free' : 'Go ' + escHtml(p.label)}</button>`;
      card.querySelector('.btn').onclick = () => id === 'free' ? showEasy(null) : openPlans();
      g.appendChild(card);
    });
  }
  document.querySelectorAll('[data-plan-price]').forEach(el => { const p = PLANS[el.dataset.planPrice]; if (p) el.textContent = p.priceLabel; });
  document.querySelectorAll('[data-plan-quota]').forEach(el => {
    const p = PLANS[el.dataset.planQuota];
    if (p) el.textContent = p.weekly ? p.weekly + ' exports a week' : p.monthly + ' exports a month';
  });
}
function buildLandingExtras(){
  fillLandingFacts();
  buildLandingFilter();
  document.querySelectorAll('#lp-vreel .vad').forEach(bindVideoTile);
  document.querySelectorAll('#lp-vreel .vreel-item').forEach(fig => {
    const vad = fig.querySelector('.vad'), id = fig.dataset.tpl;
    if (!TEMPLATES.some(t => t.id === id)) console.warn('GraphicsStudio: video tile names unknown template ' + id);
    vad.addEventListener('click', e => { if (!e.target.closest('.vad-sound,.vad-play')) showEasy(fig.dataset.tpl); });
    const cap = fig.querySelector('figcaption');
    if (cap && !cap.querySelector('.vreel-use')){
      const use = document.createElement('button');
      use.type = 'button'; use.className = 'vreel-use'; use.textContent = 'Use this template →';
      use.onclick = () => showEasy(fig.dataset.tpl);
      cap.appendChild(use);
    }
  });
}

// ── video tiles: muted autoplay, one voice at a time ─────────────────────
/* The browser rule and the design rule agree: autoplay is only allowed muted,
   and a feed ad has to work muted anyway. So every tile plays muted when at
   least half of it is on screen, pauses when it is not, and the small corner
   button is the only way sound starts — for one tile at a time. Reduced
   motion or Save-Data: nothing autoplays; the poster (frame 0, the finished
   ad) stays and a play mark offers the rest. */
const VREEL = { tiles: new Set(), io: null, hero: null };
const reelManual = () => {
  try {
...
const SOUND_BTN = '<button class="vad-sound" type="button" aria-label="Turn sound on" aria-pressed="false">' +
  '<svg class="ic ic-off"><use href="#i-sound-off"/></svg><svg class="ic ic-on"><use href="#i-sound-on"/></svg></button>';
/* assets/video/clips.json is written by scripts/render_motion_clips.mjs and is
   the one source for which files exist and where each sound button goes.
   Started at load, awaited by boot before the hero is built. */
const CLIP_MANIFEST = (typeof fetch === 'function' ? fetch('assets/video/clips.json', { cache:'no-cache' })
  .then(r => r.ok ? r.json() : []).catch(() => []) : Promise.resolve([]));
const CAN_PLAY = (() => {
  try { const v = document.createElement('video');
    return { mp4: !!v.canPlayType('video/mp4; codecs="avc1.64001F, mp4a.40.2"'), webm: !!v.canPlayType('video/webm; codecs="vp9, opus"') };
  } catch (e){ return { mp4:false, webm:false }; }
})();
function applyClipManifest(list){
  if (!Array.isArray(list) || !list.length){ console.warn('GraphicsStudio: assets/video/clips.json missing or empty, the video reel shows posters only.'); return; }
  document.querySelectorAll('#lp-vreel .vreel-item[data-clip]').forEach(fig => {
    const c = list.find(x => x.key === fig.dataset.clip);
    const v = fig.querySelector('video'), vad = fig.querySelector('.vad');
    if (!c){ console.warn('GraphicsStudio: no clip "' + fig.dataset.clip + '" in clips.json'); return; }
    fig.dataset.tpl = c.tpl;
    v.dataset.src = c.mp4; v.dataset.webm = c.webm || '';
    vad.dataset.corner = c.corner || 'br';
  });
}
function landingHeroClip(){
  const f = document.querySelector('#lp-vreel .vreel-item.fmt-square');
  const v = f && f.querySelector('video');
  return v && v.dataset.src ? { tpl: f.dataset.tpl, src: v.dataset.src, webm: v.dataset.webm, poster: v.getAttribute('poster'),
    corner: f.querySelector('.vad').dataset.corner } : null;
}
function landingVideoEl(clip, name){
  if (VREEL.hero && VREEL.hero._src === clip.src) return VREEL.hero;
  const wrap = document.createElement('div');
  wrap.className = 'vad vad-hero';
  wrap._src = clip.src;
  if (clip.corner) wrap.dataset.corner = clip.corner;
  wrap.innerHTML = `<video muted loop playsinline preload="none" poster="${escHtml(clip.poster)}" data-src="${escHtml(clip.src)}" data-webm="${escHtml(clip.webm || '')}" aria-label="Video ad: ${escHtml(name)}"></video>` + SOUND_BTN;
  bindVideoTile(wrap);
  return (VREEL.hero = wrap);
}
function reelObserver(){
  if (VREEL.io) return VREEL.io;
  if (!('IntersectionObserver' in window)) return (VREEL.io = { observe: vad => { if (!vad.classList.contains('manual')) vad._play(); } });
  VREEL.io = new IntersectionObserver(entries => entries.forEach(en => {
    const vad = en.target, v = vad.querySelector('video');
    const visible = en.isIntersecting && en.intersectionRatio >= 0.5;
    if (visible){ if (!vad.classList.contains('manual')) vad._play(); }
    else if (!v.paused){ v.pause(); if (!v.muted) setTileSound(vad, false); }
  }), { threshold: [0, 0.5] });
  return VREEL.io;
}
function setTileSound(vad, on){
  VREEL.tiles.forEach(t => {
    const tv = t.querySelector('video'), tb = t.querySelector('.vad-sound');
    const mine = t === vad, want = mine && on;
    tv.muted = !want;
    tb.setAttribute('aria-pressed', String(want));
    tb.setAttribute('aria-label', want ? 'Turn sound off' : 'Turn sound on');
  });
  if (on) vad._play();
}
function bindVideoTile(vad){
  if (!vad || vad._bound) return;
  vad._bound = true;
  const v = vad.querySelector('video'), btn = vad.querySelector('.vad-sound');
  v.muted = true; v.defaultMuted = true;       // iOS reads the property, not just the attribute
  /* The poster (frame 0, the finished ad) is also the tile's own background,
     so a video layer that paints nothing — while loading, at a loop seek, in a
     browser that drops the poster once src is set — shows the ad, never an
     empty card (rule 38). A capture of the hero caught exactly that once. */
  const poster = v.getAttribute('poster');
  if (poster){ vad.style.backgroundImage = 'url("' + poster + '")'; vad.style.backgroundSize = 'cover'; vad.style.backgroundPosition = 'center'; }
  vad.classList.add('paused');
  vad._play = () => {
    if (vad.classList.contains('noplay')) return;
    if (!v.getAttribute('src')){                        // fetched only once it is wanted
      const src = CAN_PLAY.mp4 && v.dataset.src ? v.dataset.src : CAN_PLAY.webm && v.dataset.webm ? v.dataset.webm : null;
      if (!src){ vad.classList.add('noplay'); return; }
      v.src = src;
    }
    const p = v.play();
    if (p && p.catch) p.catch(e => {
      // refused autoplay → offer the play mark; anything else → keep the poster, drop the controls
      vad.classList.add(e && e.name === 'NotAllowedError' ? 'manual' : 'noplay', 'paused');
    });
  };
  v.addEventListener('error', () => vad.classList.add('noplay'));
  v.addEventListener('playing', () => vad.classList.remove('paused'));
  v.addEventListener('pause', () => vad.classList.add('paused'));
  if (!vad.querySelector('.vad-play')){
    vad.insertAdjacentHTML('beforeend', '<button class="vad-play" type="button" aria-label="Play video"><svg class="ic"><use href="#i-play"/></svg></button>');
  }
  vad.querySelector('.vad-play').addEventListener('click', e => { e.stopPropagation(); vad._play(); });
  if (reelManual()) vad.classList.add('manual');
  btn.addEventListener('click', e => { e.stopPropagation(); e.preventDefault(); setTileSound(vad, v.muted); });
  VREEL.tiles.add(vad);
  reelObserver().observe(vad);
}

// ── wiring ───────────────────────────────────────────────────────────────
(document.readyState === 'loading' ? (f => document.addEventListener('DOMContentLoaded', f)) : (f => f()))(() => {
  const bind = (id, fn) => { const el = $(id); if (el) el.onclick = fn; };
  bind('ez-video', ezDownloadVideo);
  bind('ex-video', editorDownloadVideo);
  document.querySelectorAll('[data-video-sound]').forEach(cb => {
    cb.checked = motionSoundOn();
    cb.addEventListener('change', () => {
      jset('pgfx_video_sound', cb.checked);
      document.querySelectorAll('[data-video-sound]').forEach(o => { o.checked = cb.checked; });
    });
  });
  // no browser that can record video → say so on the button, never fail on click
  if (!motionMime(false)) document.querySelectorAll('#ez-video,#ex-video').forEach(b => {
    b.disabled = true; b.title = 'Video export needs a recent Chrome, Edge, Safari or Firefox';
  });
});
