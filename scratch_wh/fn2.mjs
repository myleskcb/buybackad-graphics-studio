export function wall_hills(x, y, w, h, P, R, id) {
  ...
  /* the ramp's turn is held to one decimal so the stop the browser paints and
     the value skyAt reports for the sun's backdrop are the same number */
  ...
  if (R.chance(.78)) {
    const band = h * sky;
    /* the ring is decided first because its stroke straddles the radius: its
       true outer edge is 1.17r, and a disc shaved flat by the card edge reads
       as a rendering fault, not as a moon */
    const ring = R.chance(.35), k = ring ? 1.17 : 1;
    /* sized off the short side, off the sky band, and off the width it has to
       clear both edges in — so a wide laptop screen does not get a disc taller
       than its sky and a narrow one does not get a disc wider than itself */
