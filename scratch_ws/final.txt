function wall_schematic(x, y, w, h, P, R, id) {
  ...
  /* a coarse survey grid — cells about a third of the short side, capped
     against the long side so a slim box never turns into texture-mush */
  const g = Math.max(m / 3.4, Math.max(w, h) / 8);
  ...
  /* camera module, upper right — lens radius is clamped so the rings keep a
     stroke's width of dark between them at every aspect ratio */
  ...
  const lr = Math.max(.6, Math.min(cross * .30, (span / (nl + 1) - s2 * 2) / 2));
  for (let i = 0; i < nl; i++) {
    const t = (i + 1) / (nl + 1);
  ...
  /* battery — the biggest, brightest shape on the screen. Its terminal tabs
     sit top-centre, clear of every ribbon pad on that edge. */
  ...
  /* ribbon runs between the blocks, right angles only, each end on a pad */
